          ...
          <div style="height: 270px;background-color: rgb(255,255,255);border-right: 1px solid #CBCBCB;padding-top: 17px;">
            <div id="calender">
              <div id="celender_head">
                ...
                <div id="celender_head_inner" class="celender_head"><p style="font-size: 18px;">{{header}}</p></div>
                  ...
                  <table width="100%">
                    ...
                    <tr v-for="(item,index) in show" :key='index'>
                      <td :class="{now: item1.now, last_month:item1.attr == 'last_month',next_month:item1.attr == 'next_month'}"
                          v-for="(item1,index) in item " @click="showDate(item1,$event);" 
                          style="height: 28px;width: 28px;position: relative" :key='index' > 
          ...
          <el-row>
            <el-col :span="12" style="border-right: 1px solid #E1E1E1; ">
              <div style="overflow-y: auto;height: 232px;">
                <ul>
                   <li class="headlineLi" style="padding-left: 23px;padding-right: 29px;line-height: 48px;height: 48px;" v-for="(item,index) in headlines" @click="headlineLiC(index)" :key='index'>
...
<script>
  ...
  export default {
    ...
    methods :{
      ...
      getcurrentDates(calender_Date) {//传入正常的Date（）类型数据  获取上个月天数
        this.initline()
      ...
      getlastDates(calenderdDate) {//传入正常的Date（）类型数据 获取下个月天数
      ...
      dialogVisibleTextarea(e){
        ...
        this.headlines.push({'name':this.inputTitle})
        this.textarea = '';
        this.inputTitle = '';
...
<style >
  ...
  #calender_body td .hongdian{
    ...
    display: inline-block;border-radius: 50%;margin-left: -2px;bottom: 3px;left: 50%;
  }
  .last_month {
    color:lightgray;
  }
